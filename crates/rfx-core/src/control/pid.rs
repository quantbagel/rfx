...
pub struct PidConfig {
    ...
    pub output_max: f64,
    /// Integral windup limit (None for no limit)
...
impl Pid {
    ...
    pub fn update_error(&mut self, error: f64, dt: f64) -> f64 {
        ...
        let alpha = self.config.derivative_filter.alpha();
        self.state.derivative_filtered =
            alpha * raw_derivative + (1.0 - alpha) * self.state.derivative_filtered;
