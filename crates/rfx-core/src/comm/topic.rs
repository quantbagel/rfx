...
impl<T: Clone + Send + 'static> Topic<T> {
    ...
    pub fn publish(&self, message: T) {
        ...
        // Update latched value if enabled
        if inner.config.latch {
            inner.latched_value = Some(message.clone());
        }

        // Send to all subscribers, removing closed ones
        inner.subscribers.retain(|tx| tx.try_send(message.clone()).is_ok());
    }
...
impl<T: Clone + Send + 'static> Publisher<T> {
    /// Publish a message
    pub fn publish(&self, message: T) {
        let mut inner = self.topic.write();
