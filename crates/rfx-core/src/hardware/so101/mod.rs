...
use types::{TimingState, HOME_POSITIONS};
...
impl So101 {
    ...
    pub fn read_positions(&self) -> Vec<f32> {
        ...
        match proto.sync_read_positions(&SERVO_IDS) {
            Ok(raw) => raw.iter().map(|&r| raw_to_radians(r)).collect(),
            Err(e) => {
                tracing::warn!("Failed to read positions: {}", e);
                self.state.read().joint_positions.to_vec()
            }
        }
    }

    /// Set target positions for all joints
    pub fn set_positions(&self, positions: Vec<f32>) -> Result<(), Error> {
        ...
        let raw_positions: Vec<u16> = positions.iter().map(|&r| radians_to_raw(r)).collect();
    ...
    pub fn go_home(&self) -> Result<(), Error> {
        let home_radians: Vec<f32> = HOME_POSITIONS.iter().map(|&r| raw_to_radians(r)).collect();
        self.set_positions(home_radians)
    }
