...
impl Go2State {
    ...
    pub fn new() -> Self {
        Self {
            motors: vec![MotorState::default(); NUM_MOTORS],
            ..Default::default()
        }
    }
    ...
    pub fn joint_positions(&self) -> [f32; NUM_MOTORS] {
        ...
        for (i, motor) in self.motors.iter().enumerate().take(NUM_MOTORS) {
            positions[i] = motor.q;
        }
        positions
    }
    ...
    pub fn joint_velocities(&self) -> [f32; NUM_MOTORS] {
        let mut velocities = [0.0f32; NUM_MOTORS];
        for (i, motor) in self.motors.iter().enumerate().take(NUM_MOTORS) {
            velocities[i] = motor.dq;
        }
        velocities
    }
}
