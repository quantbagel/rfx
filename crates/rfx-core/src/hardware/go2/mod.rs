        ...
        while connected.load(Ordering::Relaxed) {
            ...
            match state_rx.recv_timeout(Duration::from_millis(100)) {
                Ok(Some(low_state)) => {
                    ...
                    // Update motors (only first 12 for legs)
                    if s.motors.len() != NUM_MOTORS {
                        s.motors = vec![MotorState::default(); NUM_MOTORS];
                    }
                    for (i, ms) in low_state.motor_state.iter().enumerate().take(NUM_MOTORS) {
                        s.motors[i] = *ms;
                    }

                    // Update foot contact from force sensors
                    for (i, &force) in low_state.foot_force.iter().enumerate() {
                        s.foot_contact[i] = force > 20; // Threshold for contact detection
                    }
                }
                Ok(None) => {
...
impl Robot for Go2 {
    ...
    fn send_command(&self, cmd: Command) -> Result<()> {
        ...
        if let Some(positions) = cmd.positions {
            let kp = cmd.kp.unwrap_or_else(|| vec![20.0; NUM_MOTORS]);
            let kd = cmd.kd.unwrap_or_else(|| vec![0.5; NUM_MOTORS]);
